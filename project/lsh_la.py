...
def worker(input_file, partition=8):
    spark = SparkSession \
        .builder \
    ...
    def processor(names):
        ...
        similar_names = similar_names.values().flatMap(lambda x: combinations(
            sorted(x), 2)).map(lambda x: (x, 1)).reduceByKey(add)#.keys()#.map(lambda x: (min(x), {max(x)})).reduceByKey(__or__).map(lambda x:{x[0]} | x[1])
        return similar_names, singles
    ...
    with open('clusters_la_token.json', 'r') as fin:
        ...
        sample_pairs = samples.flatMap(lambda x: combinations(sorted(x), 2)).map(lambda x: (x, 1)).reduceByKey(add)#.keys()
        # print(sample_pairs.take(3))
        fin.close()

    true_positives = plurals.join(sample_pairs)
    # print(true_positives.take(3))
    print('Precision={}\tRecall={}'.format(true_positives.count()*1.0/plurals.count(), true_positives.count()*1.0/sample_pairs.count()))
    ...
    true_positives = plurals.join(sample_pairs)
    # print(true_positives.take(3))
    print('size={}\tPrecision={}\tRecall={}'.format(gram_size, true_positives.count()*1.0/plurals.count(), true_positives.count()*1.0/sample_pairs.count()))
