...
def minhash(iterable):
	...
	# print('num=',name_num, game_num)
	# exit(0)
	game_set_table = np.zeros((name_num, game_num))

	game_set = np.empty(game_num, dtype=str)

	for i, (name, game_basket) in enumerate(steam_game):
		game_set[i] = name
		game_set_table[:, i] = movie_transformation(game_set, game_basket)

	s = 50
	signature_table = np.empty((game_num, s))
	for i in range(s):
		game_set = [hash(g) for g in game_set]
		signature_table[:,i] =  game_set
...
def movie_transformation(all_games, name_baskets):
	
	game_binary_basket = [0 for _ in range(len(all_games))]
	i = j = 0
	while i < len(all_games) and j < len(name_baskets):
		if all_games[i] == name_baskets[j]:
			game_binary_basket[i] = 1
			i += 1
			j += 1
		else:
			i += 1
	return game_binary_basket
...
if __name__ == "__main__":
	...
	partition = 8
	rdd_steam = sc.textFile(file, 5)\
		.map(lambda x: x.split(','))\
		.map(lambda x: (x[1].encode('utf-8'), x[1].encode('utf-8')))
		
	# rdd_steam = spark.read.csv(file, header=True).rdd.map(tuple).repartition(partition)
	
	rdd_name = rdd_steam.map(lambda x: (str(x[1]),str(x[1])))
	name_baskets = rdd_name.groupByKey()\
		.mapValues(lambda x: sorted(list(x)))\
		.sortByKey()
	
	name_set = name_baskets.map(lambda x: x[1]).flatMap(list).distinct().collect()
	name_set = np.array(name_set)

	name_set.sort()
	name_num = len(name_set)

	minhash_result = name_baskets.mapPartitions(minhash).collect()
