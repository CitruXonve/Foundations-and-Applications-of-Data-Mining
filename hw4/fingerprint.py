...
rdd = sc.textFile("facility_name.txt", partition)
...
def translate(name):
    for i in range(len(name)):
        ch = name[i]
        new_ch = replace_ch(ch)
        if new_ch!=ch:
            name.replace(ch, new_ch)
    return name
    pass
...
def token_fingerprint(name):
    return ''.join(sorted(set(
        re.split(r'[\p{Punct}\[\]\'\-\:\s\x00-\x1f\x7f-\x9f]+', #'\\p{Punct}|[\\x00-\\x08\\x0A-\\x1F\\x7F]'
            translate(name).lower()#unicodedata.normalize('NFKD', name.lower()).encode('ascii', 'ignore')
            )
        )))
    pass

name_sorter = lambda (name, count): (-count, name)
